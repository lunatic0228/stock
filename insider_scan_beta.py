...
Beta 新增功能：
  - 顯示近20日日均交易量（張）
  - 自動過濾日均量 < 500 張的標的（流動性不足）

流程：
  ① 抓上市 + 上櫃本月大股東異動
  ② 篩選「異動數 > 0」（本月比上月多）
  ③ 每支股票跑 yfinance 技術分析 + entry_signals
  ④ 過濾日均量 < min_avg_vol 的標的
  ⑤ 輸出進場評分 + 現價位置 + 停損線 + 日均量
"""
...
def _analyze_one(code: str) -> dict | None:
    ...
    對單一股票代號跑技術分析。
    回傳 dict 或 None（資料不足）
    Beta 新增：avg_vol_lots（近20日日均成交張數）
    """
    import sys
    sys.path.insert(0, ".")
    ...
    if vol_col in df.columns:
        avg_vol_lots = int(df[vol_col].tail(20).mean() // 1000)
    else:
        avg_vol_lots = 0
    ...
    return {
        ...
        "pb":            pb,
        "div":           div,
        "avg_vol_lots":  avg_vol_lots,   # ← Beta 新增
    }


def run_insider_scan_beta(
    days_back: int = 60,
    min_lots: int = 0,
    min_avg_vol: int = 500,   # ← Beta 新增：日均量門檻（張）
) -> None:
    """
    主入口（Beta）：掃描增持名單 → 逐支分析 → 過濾低流動性 → 印出報告
    供 Streamlit 透過 redirect_stdout 捕捉輸出
    """
    # ① 取得增持名單
    ...
    total = len(df_inc)
    print(f"  共找到 {total} 筆增持紀錄，開始逐支分析（日均量 < {min_avg_vol} 張將略過）...\n")
    print("=" * 68)
    ...
    for _, row in df_inc.iterrows():
        ...
        # ── Beta：流動性過濾 ──────────────────────────────────
        if res["avg_vol_lots"] < min_avg_vol:
            skipped_vol += 1
            # 只印一行提示，不展開分析
            print(f"\n▌ {code} {name}  [{market}]  {month}")
            print(f"  ⚠  日均量 {res['avg_vol_lots']:,} 張（近20日），流動性不足（< {min_avg_vol} 張），略過")
            print("-" * 68)
            continue
        ...
        print(f"  大股東：{holder}")
        print(f"  本月增持：{lots:,} 張")
        print(f"  日均量：{res['avg_vol_lots']:,} 張（近20日）")   # ← Beta 新增
        print()
    ...
    # 摘要
    print(f"\n✅ 掃描完成")
    print(f"  有效顯示：{shown} 筆　｜　流動性過濾略過：{skipped_vol} 筆　｜　無股價資料：{skipped_data} 筆")
